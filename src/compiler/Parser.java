...
public class Parser {
	...
	public void parse() {
		...
		nxtlev.or(declarationBeginSet);
		nxtlev.or(statementBeginSet);
	...
	public void parseBlock(int level, SymSet fsys) {
		...
		do {
			...
			while (currentSymbol == Symbol.procSym) {
				...
				parseBlock(level+1, nxtlev);
				
				if (currentSymbol == Symbol.semicolon) {
					nextSymbol();
					nxtlev = (SymSet) statementBeginSet.clone();
			...
			nxtlev = (SymSet) statementBeginSet.clone();
			nxtlev.set(Symbol.ident);
			test(nxtlev, declarationBeginSet, 7);
		...
		parseStatement(nxtlev, level);
		interpreter.gen(Fct.OPR, 0, 0);		// 每个过程出口都要使用的释放数据段指令
		
		nxtlev = new SymSet(SYMBOL_NUM);	// 分程序没有补救集合
	...
	void parseVarDeclaration(int lev) {
		if (currentSymbol == Symbol.ident) {
			// 填写名字表并改变堆栈帧计数器
			table.enter(Objekt.variable, lev, dataSize);
			dataSize++;
			nextSymbol();
		} else {
